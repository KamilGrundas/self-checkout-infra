...
tail_lines="${2:-200}"
case "$service" in
  admin|backend|db|label-studio|mailcatcher|ml|ml-autolabel-worker|ml-worker|mlflow|prestart|s3-contract-test|s3-provider) ;;
...
docker compose \
  ...
  logs --no-color --tail "$tail_lines" "$service"
